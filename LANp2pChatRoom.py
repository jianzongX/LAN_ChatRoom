...
class P2P_Chat_GUI:
    def __init__(self, root):
        self.root = root
        self.root.title("P2P局域网聊天室（支持文件传输）")
        ...
        # 发送上线通知
        self.send_chat_message(f"系统消息: {self.username} 加入了聊天室（端口: {self.port}）")
    ...
    def get_user_info(self):
        ...
        # 用户名输入
        Label(dialog, text="1. 输入用户名：").pack(pady=(20, 5), anchor="w", padx=30)
        username_entry = Entry(dialog, width=35)
        username_entry.pack(pady=(0, 15), padx=30)
        username_entry.focus()

        # 端口输入
        Label(dialog, text="2. 输入通信端口（1-65535）：").pack(pady=(5, 5), anchor="w", padx=30)
    ...
    def handle_chat_message(self, msg_content, addr):
        ...
        if not msg_content.startswith(f"{self.username}:"):
            ...
            self.add_chat_log(f"[{timestamp}] {msg_content}")
    ...
    def handle_file_header(self, file_header, addr):
        ...
        try:
            ...
            if not resp:
                self.add_chat_log(f"[信息] 已拒绝接收 {sender} 的文件：{filename}")
                return
    ...
    def receive_file(self, sender_ip, port, filename, file_size):
        ...
        try:
            ...
            if received_size == file_size:
                ...
                self.add_chat_log(f"[失败] 文件接收中断（仅接收{self.format_file_size(received_size)}/{self.format_file_size(file_size)}）")

        except Exception as e:
            self.add_chat_log(f"[警告] 文件接收失败：{str(e)}")
    ...
    def select_and_send_file(self):
        ...
        try:
            ...
            self.add_chat_log(f"[信息] 已发起文件发送：{filename}（{self.format_file_size(file_size)}），等待接收方响应...")
            ...
            self.add_chat_log(f"[警告] 发起文件发送失败：{str(e)}")
    ...
    def listen_file_connections(self, file_path, filename, file_size):
        ...
        try:
            while self.running and client_count < 5:  # 最多同时发给5个用户
                ...
                client_count += 1
                self.add_chat_log(f"[连接] 已连接接收方：{client_addr[0]}（{filename}）")
            ...
            if client_count == 0:
                self.add_chat_log(f"[超时] 文件发送超时：30秒内无接收方响应")
        except Exception as e:
            self.add_chat_log(f"[警告] 文件连接监听错误：{str(e)}")
        finally:
            self.is_transferring = False
            self.hide_transfer_progress()
            self.add_chat_log(f"[信息] 文件发送任务结束（共发送给{client_count}个接收方）")
    ...
    def send_file_to_client(self, client_sock, file_path, filename, file_size, client_addr):
        ...
        try:
            ...
            if sent_size == file_size:
                self.add_chat_log(f"[成功] 已成功发送文件给：{client_addr[0]}（{filename}）")
            else:
                self.add_chat_log(f"[失败] 发送给{client_addr[0]}失败：仅发送{self.format_file_size(sent_size)}/{self.format_file_size(file_size)}")

        except Exception as e:
            self.add_chat_log(f"[警告] 发送给{client_addr[0]}错误：{str(e)}")
        finally:
            client_sock.close()


    # ---------------------- 辅助函数 ----------------------
    def send_chat_message(self, system_msg=None):
        """发送聊天消息（支持系统消息）"""
        # 如果是用户输入的消息
        ...
        try:
            ...
            if system_msg is None:
                timestamp = datetime.now().strftime("%H:%M:%S")
                self.add_chat_log(f"[{timestamp}] 我：{msg}")
        except Exception as e:
            self.add_chat_log(f"[警告] 消息发送失败：{str(e)}")


    def add_chat_log(self, content):
        """添加内容到聊天记录区（线程安全）"""
        def _update():
    ...
    def on_exit(self):
        ...
        # 发送下线通知
        try:
            self.send_chat_message(f"系统消息: {self.username} 离开了聊天室")
